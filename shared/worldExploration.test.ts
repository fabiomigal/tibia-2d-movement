...
describe("expansão de exploração do Vale de Âmbar", () => {
  it("mantém os portais bidirecionais com destino e regiões declaradas", () => {
    ...
    expect(WORLD_PORTALS).toHaveLength(2);
  ...
  it("define spawns únicos, combatíveis e com respawn de exatamente dois segundos", () => {
    ...
    // Com a expansão, temos múltiplos spawns do mesmo monstro, mas cada um deve ser único por posição
    const uniqueSpawns = new Set(WORLD_MONSTER_SPAWNS.map((spawn) => `${spawn.monsterKey}-${spawn.x}-${spawn.z}`));
    expect(uniqueSpawns.size).toBe(WORLD_MONSTER_SPAWNS.length);
